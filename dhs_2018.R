...
# Births
setwd(paste0(datawd,'/data/dhs_2018/SNBR80DT/'))
temp              <- read.dhs('SNBR80FL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2018/SNHR80DT/'))
temp              <- read.dhs('SNHR80FL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2018/SNPR80DT/'))
temp                 <- read.dhs('SNPR80FL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2018/SNIR80DT/'))
temp              <- read.dhs('SNIR80FL','')
female            <- temp$file
...
setwd(datawd)
saveRDS(dhs_2018, file = "./data/midsave/dhs_2018.rds")

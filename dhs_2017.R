...
# Births
setwd(paste0(datawd,'/data/dhs_2017/SNBR7ZDT/'))
temp              <- read.dhs('SNBR7ZFL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2017/SNHR7ZDT/'))
temp              <- read.dhs('SNHR7ZFL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2017/SNPR7ZDT/'))
temp                 <- read.dhs('SNPR7ZFL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2017/SNIR7ZDT/'))
temp              <- read.dhs('SNIR7ZFL','')
female            <- temp$file
...
setwd(datawd)
saveRDS(dhs_2017, file = "./data/midsave/dhs_2017.rds")

...
# Births
setwd(paste0(datawd,'/data/dhs_2016/SNBR7IDT/'))
temp              <- read.dhs('SNBR7IFL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2016/SNHR7IDT/'))
temp              <- read.dhs('SNHR7IFL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2016/SNPR7IDT/'))
temp                 <- read.dhs('SNPR7IFL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2016/SNIR7IDT/'))
temp              <- read.dhs('SNIR7IFL','')
female            <- temp$file
...
setwd(datawd)
saveRDS(dhs_2016, file = "./data/midsave/dhs_2016.rds")

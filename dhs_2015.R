...
# Load surveys ------------------------------------------------------------
# https://www.dhsprogram.com/what-we-do/survey/survey-display-524.cfm
# For variable coding: https://dhsprogram.com/pubs/pdf/DHSG4/Recode6_Map_22March2013_DHSG4.pdf

# Births
setwd(paste0(datawd,'/data/dhs_2015/SNBR7HDT/'))
temp              <- read.dhs('SNBR7HFL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2015/SNHR7HDT/'))
temp              <- read.dhs('SNHR7HFL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2015/SNPR7HDT/'))
temp                 <- read.dhs('SNPR7HFL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2015/SNIR7HDT/'))
temp              <- read.dhs('SNIR7HFL','')
female            <- temp$file
...
setwd(datawd)
saveRDS(dhs_2015, file = "./data/midsave/dhs_2015.rds")

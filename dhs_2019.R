...
# Load surveys ------------------------------------------------------------
# https://www.dhsprogram.com/what-we-do/survey/survey-display-534.cfm
# For variable coding: https://www.dhsprogram.com/pubs/pdf/DHSG4/Recode7_Map_31Aug2018_DHSG4.pdf

# Births
setwd(paste0(datawd,'/data/dhs_2019/SNBR8ADT/'))
temp              <- read.dhs('SNBR8AFL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2019/SNHR8ADT/'))
temp              <- read.dhs('SNHR8AFL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2019/SNPR8ADT/'))
temp                 <- read.dhs('SNPR8AFL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2019/SNIR8ADT/'))
temp              <- read.dhs('SNIR8AFL','')
female            <- temp$file
...
ind6 <- hhs %>% 
  ...
  drop_na %>% 
  mutate(ind6 = factor(x = ifelse((v205 %in% c(23, 31, 41, 42, 43) | v225 == 1), 1, 0), levels = c(0,1), labels = c('not_deprived', 'deprived'))) %>% 
  select(hhid, ind6) %>% 
  distinct(hhid, .keep_all = TRUE)
...
ind9 <- hhs %>% 
  ...
  select(hhid, v213) %>% 
  drop_na %>% 
  mutate(ind9 = factor(x = ifelse(v213 %in% c(11, 12), 1, 0), levels = c(0,1), labels = c('not_deprived', 'deprived'))) %>% 
  select(hhid, ind9) %>% 
  distinct(hhid, .keep_all = TRUE) 
...
setwd(datawd)
saveRDS(dhs_2019, file = "./data/midsave/dhs_2019.rds")

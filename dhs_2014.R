...
# Births
setwd(paste0(datawd,'/data/dhs_2014/SNBR70DT/'))
temp              <- read.dhs('SNBR6RFL','h*')
births            <- temp$file
births_var_labels <- temp$crosswalk
rm(temp)

# Households
setwd(paste0(datawd,'/data/dhs_2014/SNHR70DT/'))
temp              <- read.dhs('SNHR6RFL','h*')
hhs               <- temp$file
hhs_var_labels    <- temp$crosswalk
rm(temp)

# Household members
setwd(paste0(datawd,'/data/dhs_2014/SNPR70DT/'))
temp                 <- read.dhs('SNPR6RFL','h*')
hhmembers            <- temp$file
hhmembers_var_labels <- temp$crosswalk
rm(temp)

# Individuals (women and children)
setwd(paste0(datawd,'/data/dhs_2014/SNIR70DT/'))
temp              <- read.dhs('SNIR6RFL','')
female            <- temp$file
...
setwd(datawd)
saveRDS(dhs_2014, file = "./data/midsave/dhs_2014.rds")
